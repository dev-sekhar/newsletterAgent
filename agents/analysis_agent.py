...
class AnalysisAgent:
    ...
    def execute(self, articles: list[dict]) -> tuple[dict, str]:
        ...
        processed_articles = []
        skipped_count = 0
        total_to_process = len(articles) - sum(1 for article in articles if is_url_processed(article.get('url', '')))

        for article in articles:
            if is_url_processed(article.get('url')): continue
            if not article.get('title') or "[Removed]" in article.get('title'):
                skipped_count += 1
                continue

            try:
                clean_description = self._sanitize_description(article.get('description', ''))
        ...
        explanation = (
            f"Analysis Agent received {total_to_process} new articles to process. "
            f"It successfully analyzed and enriched {len(processed_articles)} of them. "
            f"{skipped_count} articles were skipped due to being malformed or processing errors."
        )
        print(f"  > {explanation}")
    ...
    def _categorize(self, title: str) -> str:
        prompt = f"Categorize the following article title into ONE of these categories: {self.config['categories']}. Title: \"{title}\". Return a JSON object with one key: \"category\". Example: {{\"category\": \"DeFi\"}}"
        response = self.groq_client.chat.completions.create(model=self.fast_model, messages=[{"role": "user", "content": prompt}], temperature=0, response_format={"type": "json_object"})
        category = json.loads(response.choices[0].message.content).get("category", "Other")
        if category not in self.config['categories']: category = "Other"
        return category

    def _summarize(self, title: str, description: str) -> str:
        prompt = f"Write a concise, 3-sentence summary for the following article. Title: \"{title}\". Description: \"{description}\". Return a JSON object with one key: \"summary\". Example: {{\"summary\": \"This is a summary.\"}}"
        response = self.groq_client.chat.completions.create(model=self.fast_model, messages=[{"role": "user", "content": prompt}], temperature=0, response_format={"type": "json_object"})
