...
class ContentCreationAgent:
    ...
    def execute(self, articles):
        ...
        print(f"  > Processed and created content for {new_articles_found} new articles.")
        return classified_content, new_articles_found

    def _classify_and_summarize(self, article):
        prompt = f"""You are an expert tech news analyst... (rest of prompt)""" # Same prompt as before
        try:
            ...
            if result.get("category") not in self.categories: result["category"] = "Other"
            return result
        except Exception as e:
            print(f"  > Content creation failed for article '{article['title']}': {e}")
            return None
