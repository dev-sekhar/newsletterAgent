...
class ReputationAgent:
    ...
    def execute(self, all_articles, candidate_sources):
        ...
        approved_sources = self._get_reputable_sources_from_llm(candidate_sources)
        ...
        print(f"  > Filtered down to {len(filtered_articles)} articles from LLM-approved sources.")
        return filtered_articles

    def _get_reputable_sources_from_llm(self, sources):
        prompt = f"""You are a meticulous senior news editor... (rest of prompt)""" # Same prompt as before
        try:
            response = self.client.chat.completions.create(model=self.model_name, messages=[{"role": "user", "content": prompt}], temperature=0, response_format={"type": "json_object"})
            result = json.loads(response.choices[0].message.content)
            approved_list = result.get("approved_sources", [])
            print(f"  > LLM approved {len(approved_list)} sources.")
            return approved_list
        except Exception as e:
            print(f"  > LLM reputation check failed: {e}. Falling back to all candidates.")
            return sources
