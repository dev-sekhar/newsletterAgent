...
class NewsletterPipeline:
    ...
    def curate_selection(self, categorized_content):
        ...
        try:
            ...
            print(f"  > Curation LLM approved {len(reputable_sources)} sources.")
        except Exception as e:
...
# --- SHARED STATE & TOOL DEFINITIONS ---

# A simple dictionary to act as a shared memory/context for the tools.
CONTEXT = {
    "draft": None,
    "review": None,
}
...
def create_newsletter_draft(keyword: str) -> str:
    """
    Creates a complete draft of the newsletter and saves it to a shared context for other tools to use. This must be the first step in the process.
    Input is the topic keyword. Returns a success or error message.
    """
    ...
    CONTEXT["draft"] = markdown_draft  # Save the draft to the shared context
    return "Successfully created a newsletter draft and saved it to the context for review."

@tool
def review_newsletter_draft(topic: str) -> str:
    """
    Reviews the newsletter draft that is stored in the shared context. This is a critical quality check step that must be performed after creating a draft.
    Input is the original topic string to check for relevance.
    Returns the review decision and saves it to the context.
    """
    print("\nTOOL: `review_newsletter_draft` called.")
    global CONTEXT
    newsletter_draft = CONTEXT.get("draft")
    if not newsletter_draft:
        return "Error: No draft found in the context to review. You must run `create_newsletter_draft` first."
    ...
    try:
        ...
        CONTEXT["review"] = review_result  # Save the review decision to the context
        return f"Review complete. The decision is '{review_result.get('decision')}'. Reason: {review_result.get('reason')}"
    except Exception as e:
        CONTEXT["review"] = {"decision": "reject", "reason": f"An error occurred during review: {e}"}
        return f"An error occurred during review: {e}"

@tool
def publish_final_newsletter() -> str:
    """
    Publishes the newsletter to a file, but only if the draft has been reviewed and approved. This should be the very last step.
    It reads the draft and review decision from the shared context. This tool takes no input.
    """
